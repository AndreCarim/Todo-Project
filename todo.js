...
todoInput.onkeyup = ((e) => {
    ...
    if(value && e.keyCode === 13){
        //this means that user pressed enter
        addTodo(value)
...
function render(){
    let unCompletedaTodos = todoItems.filter(item => !item.completed)
    let completedTodos = todoItems.filter(item => item.completed)

    completedTodosDiv.innerHTML = ''
    uncompletedTodosDiv.innerHTML = ''

    if(unCompletedaTodos.length > 0){
        unCompletedaTodos.forEach(todo => {
